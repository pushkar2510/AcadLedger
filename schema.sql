...
CREATE FUNCTION public.check_username_available(p_username text, p_user_id uuid) RETURNS boolean
...
ALTER FUNCTION public.check_username_available(p_username text, p_user_id uuid) OWNER TO postgres;

--
-- Name: get_candidate_home_stats(uuid); Type: FUNCTION; Schema: public; Owner: supabase_admin
--

CREATE FUNCTION public.get_candidate_home_stats(p_profile_id uuid) RETURNS jsonb
...
CREATE FUNCTION public.grade_attempt(p_attempt_id uuid) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
...
ALTER FUNCTION public.grade_attempt(p_attempt_id uuid) OWNER TO supabase_admin;

--
-- Name: grade_attempt_v2(uuid, integer); Type: FUNCTION; Schema: public; Owner: supabase_admin
--

CREATE FUNCTION public.grade_attempt_v2(p_attempt_id uuid, p_final_time_spent integer) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION public.grade_attempt_v2(p_attempt_id uuid, p_final_time_spent integer) OWNER TO supabase_admin;
...
CREATE FUNCTION public.handle_session_sync() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
ALTER FUNCTION public.handle_session_sync() OWNER TO supabase_admin;

--
-- Name: init_test_attempt(uuid); Type: FUNCTION; Schema: public; Owner: supabase_admin
--

CREATE FUNCTION public.init_test_attempt(p_test_id uuid) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION public.init_test_attempt(p_test_id uuid) OWNER TO supabase_admin;
...
CREATE FUNCTION public.revoke_sessions_batch(p_session_ids uuid[]) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'auth'
    AS $$
BEGIN
...
ALTER FUNCTION public.revoke_sessions_batch(p_session_ids uuid[]) OWNER TO supabase_admin;

--
-- Name: save_answer(uuid, uuid, uuid[], integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.save_answer(p_attempt_id uuid, p_question_id uuid, p_selected_option_ids uuid[], p_time_spent_seconds integer DEFAULT 0) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
begin
...
ALTER FUNCTION public.save_answer(p_attempt_id uuid, p_question_id uuid, p_selected_option_ids uuid[], p_time_spent_seconds integer) OWNER TO postgres;

--
-- Name: save_test_v2(uuid, jsonb, jsonb[], text); Type: FUNCTION; Schema: public; Owner: supabase_admin
--

CREATE FUNCTION public.save_test_v2(p_test_id uuid, p_settings jsonb, p_questions jsonb[], p_status text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_user_id uuid := auth.uid();
...
ALTER FUNCTION public.save_test_v2(p_test_id uuid, p_settings jsonb, p_questions jsonb[], p_status text) OWNER TO supabase_admin;
...
CREATE FUNCTION public.sync_candidate_profile() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
begin
...
CREATE FUNCTION public.sync_user_session() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
begin
...
ALTER FUNCTION public.sync_user_session() OWNER TO postgres;
...
CREATE VIEW public.attempt_details AS
 SELECT ta.id,
    ta.test_id,
    ta.student_id,
...
ALTER TABLE public.attempt_details OWNER TO postgres;
...
CREATE VIEW public.tag_performance AS
...
ALTER TABLE public.tag_performance OWNER TO postgres;
...
CREATE VIEW public.view_question_analysis AS
...
ALTER TABLE public.view_question_analysis OWNER TO postgres;

--
-- Name: view_test_results_detailed; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.view_test_results_detailed AS
...
ALTER TABLE public.view_test_results_detailed OWNER TO postgres;

--
-- Name: view_test_summary; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.view_test_summary AS
 SELECT t.id,
    t.title,
    t.description,
    t.recruiter_id,
...
CREATE TRIGGER trg_attempt_answers_updated_at BEFORE UPDATE ON public.attempt_answers FOR EACH ROW EXECUTE FUNCTION extensions.moddatetime('updated_at');
...
CREATE TRIGGER trg_attempts_updated_at BEFORE UPDATE ON public.test_attempts FOR EACH ROW EXECUTE FUNCTION extensions.moddatetime('updated_at');
...
CREATE TRIGGER trg_candidate_profiles_sync AFTER INSERT OR UPDATE OF first_name, middle_name, last_name, profile_image_path ON public.candidate_profiles FOR EACH ROW EXECUTE FUNCTION public.sync_candidate_profile();
...
CREATE TRIGGER trg_candidate_profiles_updated_at BEFORE UPDATE ON public.candidate_profiles FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TRIGGER trg_questions_updated_at BEFORE UPDATE ON public.questions FOR EACH ROW EXECUTE FUNCTION extensions.moddatetime('updated_at');
...
CREATE TRIGGER trg_tests_updated_at BEFORE UPDATE ON public.tests FOR EACH ROW EXECUTE FUNCTION extensions.moddatetime('updated_at');
